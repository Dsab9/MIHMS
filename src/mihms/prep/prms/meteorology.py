...
from mihms.prep.utils import (
    check_precip_string,
    check_min_temp_string,
    check_max_temp_string,
)

def ea(temp_c: np.ndarray) -> np.ndarray:
    """
    Saturation vapor pressure (hPa) as a function of temperature (°C).

    PRMS / Jensen–Haise formulation.
    """
    return 6.11 * np.exp((17.27 * temp_c) / (temp_c + 237.3))


def temp_1sta_from_gridded(
    gridded_data: Union[str, Path, xr.Dataset],
    elev_data,
    model_grid: gpd.GeoDataFrame,
    temp_units: str = "celcius",
    precip_units: str = "mm",
    station_limit: Optional[int] = None,
):
    """
    Build PRMS 1-station meteorology forcings from gridded inputs
    using pywatershed conventions.

    Returns
    -------
    param_ds : xr.Dataset
        PRMS parameter dataset
    forcing_ds : xr.Dataset
        Time-series forcing dataset (tmin, tmax, precip)
    """

    # ------------------------------------------------------------------
    # Load gridded data
    # ------------------------------------------------------------------
    if not isinstance(gridded_data, xr.Dataset):
        gridded_data = xr.open_dataset(gridded_data)

    # Identify variables
    var_map = {}
    for v in gridded_data.data_vars:
    ...
    ds = gridded_data.rename(var_map)

    # ------------------------------------------------------------------
    # Grid intersection → station definition
    # ------------------------------------------------------------------
    grid_vec = vectorize_grid(ds)
    grid_vec = grid_vec.to_crs(model_grid.crs)

    grid_cent = model_grid.copy()
    grid_cent["geometry"] = grid_cent.centroid

    insct = grid_cent.sjoin(grid_vec, predicate="within")
    station_ids = insct["index_right"].unique()
    station_ids.sort()

    nstations = len(station_ids)

    # Map model HRUs → stations
    hru_tsta = (
        insct["index_right"]
        .replace({v: i + 1 for i, v in enumerate(station_ids)})
        .values
    )

    # ------------------------------------------------------------------
    # Elevation per station
    # ------------------------------------------------------------------
    elev_stats = calc_zonal_stats(grid_vec, elev_data, stats="median", all_touched=True)
    tsta_elev = elev_stats.loc[(station_ids, 1, "median")].values.ravel()

    # ------------------------------------------------------------------
    # Extract station time-series
    # ------------------------------------------------------------------
    t, ny, nx = ds.precip.shape
    met_shape = (ny, nx)

    sr, sc = np.unravel_index(station_ids, met_shape)

    tmin = ds.tmin.values[:, sr, sc]
    tmax = ds.tmax.values[:, sr, sc]
    precip = ds.precip.values[:, sr, sc]

    time = pd.DatetimeIndex(ds.time.values)

    forcing_ds = xr.Dataset(
        data_vars=dict(
            tmin=(("time", "nstations"), tmin),
            tmax=(("time", "nstations"), tmax),
            precip=(("time", "nstations"), precip),
        ),
        coords=dict(
            time=time,
            nstations=np.arange(1, nstations + 1),
        ),
        attrs=dict(
    ...
    # ------------------------------------------------------------------
    # PRMS parameters (pywatershed format)
    # ------------------------------------------------------------------
    nhru = len(model_grid)

    param_ds = xr.Dataset(
        data_vars=dict(
            basin_tsta=(("one",), np.array([0], dtype=int)),
            hru_tsta=(("nhru",), hru_tsta.astype(int)),
            hru_psta=(("nhru",), hru_tsta.astype(int)),
            tsta_elev=(("nstations",), tsta_elev),
            temp_units=(("one",), np.array([1 if temp_units == "celcius" else 0])),
            precip_units=(("one",), np.array([1 if precip_units == "mm" else 0])),
            ppt_zero_thresh=(("one",), np.array([0.0])),
            max_missing=(("one",), np.array([3])),
        ),
        coords=dict(
            nhru=np.arange(nhru),
            nstations=np.arange(1, nstations + 1),
...
def lapse_rates_from_gridded(
    temp_grid: xr.DataArray,
    elev,
) -> xr.DataArray:
    """
    Calculate monthly temperature lapse rates from gridded temperature
    and elevation using first-order spatial gradients.

    Returns
    -------
    xr.DataArray
        Dimensions: (month, y, x)
        Units: degC / 1000 elevation units
    """

    grid_vec = vectorize_grid(temp_grid)
    elev_zon = calc_zonal_stats(
        grid_vec,
        elev,
        stats="median",
        all_touched=True,
    ...
    medelev = (
        elev_zon
        .loc[(slice(None), slice(None), "median")]
        .iloc[:, 0]
        .values
        .reshape(temp_grid.shape[1:])
    )

    gradx, grady = np.gradient(medelev)
    elev_grad = np.mean([gradx, grady], axis=0)

    mon_temp = temp_grid.groupby("time.month").mean("time")

    lapse = []
    for i in range(mon_temp.sizes["month"]):
        tx, ty = np.gradient(mon_temp.values[i])
        tgrad = np.mean([tx, ty], axis=0)
        lapse.append((tgrad / elev_grad) * 1000.0)

    lapse = xr.DataArray(
        np.stack(lapse),
        coords=mon_temp.coords,
        dims=mon_temp.dims,
        name="t_lapse_rate",
        attrs={"units": "degC / 1000 elevation_units"},
    )

    return lapse

def mean_temp_warmest_month_by_hru(
    forcing_ds: xr.Dataset,
    hru_tsta: np.ndarray,
) -> tuple[np.ndarray, np.ndarray]:
    """
    Compute mean min/max temperature for the warmest month,
    mapped to each HRU.

    Parameters
    ----------
    forcing_ds : xr.Dataset
        Must contain variables `tmin` and `tmax`
        Dimensions: (time, nstations)
    hru_tsta : np.ndarray
        Station index per HRU (1-based)

    Returns
    -------
    tmin_hru, tmax_hru : np.ndarray
        Arrays of length nhru
    """

    tmean = (forcing_ds.tmin + forcing_ds.tmax) / 2.0
    monthly = tmean.groupby("time.month").mean("time")

    warmest_month = monthly.mean("nstations").argmax("month").item()

    tmin_month = (
        forcing_ds.tmin
        .groupby("time.month")
        .mean("time")
        .sel(month=warmest_month + 1)
        .values
    )
    ...
    idx = hru_tsta.astype(int) - 1

    return tmin_month[idx], tmax_month[idx]


def jh_temperature_coefficient(
    hru_elev: np.ndarray,
    hru_mintemp: np.ndarray,
    hru_maxtemp: np.ndarray,
    elev_scalar: float = 1.0,
) -> np.ndarray:
    """
    Jensen-Haise temperature coefficient for PRMS ET.

    All inputs are HRU-based arrays.
    """

    return (
        27.5
        - 0.25 * (ea(hru_maxtemp) - ea(hru_mintemp))
        - (hru_elev * elev_scalar) / 1000.0
    )
